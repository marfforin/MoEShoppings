...
-(void)addScroll1{
    ...
    self.scroll1 =[[UIScrollView alloc]initWithFrame:CGRectMake(0, 0, screen_width, self.xl_topHeight?self.xl_topHeight:35)];
...
//            NSLog(@"%f",self.xl_topHeight?self.xl_topHeight:50);
        }else {
            temp.frame =CGRectMake(screen_width/5*i, 0, screen_width/5, self.xl_topHeight?self.xl_topHeight:40);
        }
    ...
    if (self.xl_isSliderCorner) {
        [self.view2.layer setCornerRadius:self.xl_sliderCorner?self.xl_sliderCorner:1];
    }
...
-(void)addScroll2{
    ...
    for (int i =0; i<self.xl_buttonNames.count; i++) {
        ...
        if (!self.xl_views) {
            ...
            temp.frame =(CGRect){{screen_width * i, 0},self.scroll2.frame.size.width};
            [self.scroll2 addSubview:temp];
        }
    }
